    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            if (error.message === 'Firebase: Error (auth/email-already-in-use).') {
                seteEror('El correo ya está en uso');
            }
        }
    };
    ...
    return (
        <div className='w-screen h-screen flex justify-center items-center bg-blue-50'>
            ...
            <form onSubmit={handleRegister} className='flex flex-col w-lg h-1/2 justify-center items-center gap-5 rounded-2xl shadow-2xl border-0.5 bg-white'>
                ...
                <Link className="text-blue-300" to='/login'>Si ya tienes usuario, inicia sesión 🙈</Link>
                <label className='text-xl'>
                    Email:
                    <input
                        value={email}
                        onChange={(e) => setEmail(e.target.value)}
                        className='border-1 ml-1 rounded-2xl'
                        type="email"
                        name="email"
                    />
                </label>
                ...
                <button className='bg-blue-600 py-3 px-4 text-3xl rounded-2xl cursor-pointer' type="submit">
                    Register
                </button>
            </form>
            <button
                onClick={handleGoogleSignIn}
                className='bg-red-600 py-3 px-4 text-3xl rounded-2xl cursor-pointer mt-4'
            >
